...
import { ClientInfoPost } from '../../api/service';

const Input = styled('input')({
  display: 'none',
});

const FormComponent = ({ initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      setValue('copyright', initialData.copyright || '');
      if (initialData.logo) {
        setLogoPreview(initialData.logo);
      }
      if (initialData.slideshow) {
        setSlideshow(Array.isArray(initialData.slideshow) ? initialData.slideshow : JSON.parse(initialData.slideshow));
      }
    }
    ...
    const slideshowBase64 = await Promise.all(
      slideshow.map(async (item) => {
        if (item.file) {
          return {
            url: item.url,
            image: await toBase64(item.file)
          };
        }
        return item;
      })
